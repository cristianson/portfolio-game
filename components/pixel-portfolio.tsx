...
import { ArrowUp, ArrowDown, ArrowLeft, ArrowRight, X, Gamepad2 } from 'lucide-react'
    ...
    content: {
      ...
      body: (
        <div className="space-y-4">
          ...
          <p>With a background in both UX research and UI design, I bridge the gap between user needs and business goals.</p>
      ...
      body: (
        <div className="grid gap-6">
          <div className="border-2 border-black p-4 bg-white text-black">
            ...
            <p className="text-sm">A complete overhaul of a fashion retailer's mobile checkout flow, resulting in a 15% increase in conversion.</p>
          </div>
          <div className="border-2 border-black p-4 bg-white text-black">
            <h3 className="text-lg mb-2 font-bold">Finance Dashboard</h3>
            <p className="text-xs mb-2">Product Design • 2023</p>
            <p className="text-sm">Designed a complex data visualization tool for fintech analysts to track market trends in real-time.</p>
          </div>
        </div>
      )
    }
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-sm leading-relaxed">
              An in-depth look at how we solved the retention problem for a social media startup through gamification and improved onboarding flows.
            </p>
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed">
              How implementing WCAG 2.1 standards from day one improved the overall usability for all users, not just those with disabilities.
            </p>
            <button className="mt-2 text-xs underline hover:text-orange-400">Read full study &gt;</button>
          </div>
        </div>
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden select-none touch-none">
      ...
      >
        ...
        {ZONES.map(zone => (
          ...
          >
            <div className={`w-16 h-16 ${zone.color} pixel-border animate-bounce flex items-center justify-center text-2xl`}>
              {zone.icon}
            </div>
            ...
            {/* Interaction Ring */}
            <div className={`absolute inset-0 -m-4 border-2 border-dashed rounded-full opacity-30 ${activeZone === zone.id ? 'border-white animate-spin-slow' : 'border-transparent'}`} />
          </div>
        ))}
        ...
        >
          ...
          <div className={`w-full h-full relative transition-transform ${gameState.direction === 'LEFT' ? 'scale-x-[-1]' : ''}`}>
             {/* Body */}
             ...
             {/* Legs Animation */}
             <div className={`absolute -bottom-1 left-2 w-3 h-3 bg-black ${gameState.isMoving ? 'animate-pulse' : ''}`}></div>
             <div className={`absolute -bottom-1 right-4 w-3 h-3 bg-black ${gameState.isMoving ? 'animate-pulse delay-75' : ''}`}></div>
          </div>
      ...
      {isModalOpen && currentZoneData && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-0 md:p-4 animate-in fade-in duration-200">
          <div className="bg-white text-black w-full h-full md:w-full md:max-w-2xl md:h-auto md:max-h-[80vh] flex flex-col md:pixel-border shadow-2xl animate-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className={`${currentZoneData.color} p-4 flex justify-between items-center border-b-4 border-black shrink-0`}>
